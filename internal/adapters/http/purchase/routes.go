...
func RegisterRoutes(r chi.Router, h *Handler, authorizers ...auth.Authorizer) {
	...
	r.Route("/purchase-orders", func(pr chi.Router) {
		...
		access(auth.ActionWrite).Post("/{id}/alternatives", h.CreateAlternativeGroup)
		access(auth.ActionRead).Get("/{id}/alternatives", h.ListAlternativeOrders)
		access(auth.ActionWrite).Delete("/{id}/alternatives", h.ClearAlternativeGroup)
