...
graph TD
    ...
    style P23 fill:#2b2d42,stroke:#8d99ae,color:#fff
...
**الحالة التنفيذية:** مكتملة للمصروفات برمجياً، واختبارات PostgreSQL الأساسية ناجحة. تم تنفيذ نموذج `Expense` الموحد وسلوك Odoo 19 للمراحل Domain/HR، التخزين، Use Cases، الضرائب والمحاسبة، HTTP، ACL، وActivity عند Submit. الفحص اليدوي لرفع Receipt ودورة API الكاملة متبقٍ، و`go test ./...` متوقف بسبب أخطاء Loyalty خارج المرحلة 19.

الخطة المرحلية التفصيلية ومعايير الانتقال والإغلاق موجودة في [خطة تنفيذ المرحلة 19](phase19_expenses_execution_plan.md). اجتازت الدورة الآلية `Create -> Submit -> Approve -> Post` واختبارات الضرائب والتكرار والتقسيم والقيد المتوازن، وتم التحقق من migrations وقيود PostgreSQL وفهارس checksum.

**تقدير الجهد**: ~8-12 يوم عمل للتنفيذ والاختبارات الصارمة
...
**الحالة التنفيذية:** مكتملة ضمن نطاق Odoo 19 المعتمد. تم تنفيذ دورة Purchase Requisition للنوعين `blanket_order` و`purchase_template`، Supplier Info، company scope، إنشاء RFQ/PO، Alternative PO Groups، HTTP، ACL، migrations، واختبارات المطابقة الأساسية. تفاصيل التنفيذ والأدلة في [خطة تنفيذ المرحلة 20](phase_20_purchase_requisition_implementation_plan.md).

**ملاحظة الاختبارات:** اختبارات المرحلة 20 المركزة وmigrations PostgreSQL تمر. `go test ./...` ما زال متوقفًا بسبب أخطاء خارج المرحلة في موديولي Delivery وLoyalty.

**تقدير الجهد الفعلي**: ~8-12 يوم عمل للتنفيذ والاختبارات الصارمة
...
**الحالة التنفيذية:** مكتملة ضمن نطاق Odoo 19 المعتمد. تم تنفيذ الأنواع الثمانية للبرامج، محرك النقاط (order/money/unit)، المكافآت (خصم/منتج/نقطة-لكل-نقطة)، البطاقات والنقاط والسجل، تكامل confirm/cancel لأوامر البيع (Settle/Reverse/Release)، ACL والمigrations. تفاصيل التنفيذ والأدلة في [خطة تنفيذ المرحلة 23](phase_23_loyalty_implementation_plan.md).
...
| **19** | إدارة المصروفات | M3, M9, M10, M15 | 8-12 يوم عمل | 🟡 مهم | ✅ |
